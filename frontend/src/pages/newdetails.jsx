...
const NewDetails = () => {
    ...
    function FindNewArticle() {
        ArticleClient.FindNewest(pageView.current, Constants.PageSize).then((data) => {
            if (!data.status) {
            ...
            setPageView((origin) => {
                return {...origin, total: data.data.total}
            });
            setIsLoading(false);
        })
    }
    ...
    return (
        <div className=" w-full h-full">
            ...
            <Header/>
            {/**热点文章内容主体 */}
            {isLoad ? <div className='w-full h-full flex justify-center items-center'><Spin isSpin={isLoad}
                                                                                            className=" w-20 h-20"/>
                ...
                {isEmpty ? <div className=' h-screen flex justify-center items-center'><Empty className=' pt-24'
                    ...
                    <div className=" w-full mt-8">
                        {searchArticles.map((item, index) => (<div
                    ...
                    <Pagination align="end" onChange={(page) => {
                        setPageView((origin) => ({total: origin.total, current: page}))
                    }} current={pageView.current} pageSize={Constants.PageSize} defaultCurrent={pageView.current}
