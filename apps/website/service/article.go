...
func init() {
	...
	regexpImage, err = regexp.Compile(`!\[.*?\]\((.*?filename=([\w\-.]+))\s`)
	if err != nil {
...
func (a *article) MatchMdContextImage(context string) ([]string, error) {
	images := a.matchImage.FindAllStringSubmatch(context, -1)
	var realPictures = []string{}
	var err error
	for i := 0; i < len(images); i++ {
		if len(images[i]) < 2 {
			logrus.Errorf("image regexp match failed:%s", context)
			err = errors.New("image regexp match failed")
			return nil, err
		}
		realPictures = append(realPictures, images[i][2])
	}
	return realPictures, err
}
...
func (a *article) PublishArticle(ctx context.Context, article *model.Article) (id uint, err error) {
	var realPictures []string
	realPictures, err = a.MatchMdContextImage(article.Content)
	if err != nil {
		return
	}
