...
func (a *article) GetAllArticleImages(ctx context.Context) (res []string, err error) {
	storage := db.GetMysql()
	row := make([]string, 0)
	err = storage.Model(&model.Article{}).Select("images").Find(&row).Error
	if err != nil {
		logrus.Errorf("find all images from article failed:%s", err.Error())
		return
	}
